...
| `90-ip-speed-check.lst` | 58 | Проверка IP и DNS-утечек. Напрямую — иначе покажут адрес туннеля вместо реального |
...
| `20-forced-tunnel.lst` | 1 | Уводим в туннель намеренно. `ooklaserver.net` — тест-серверы Ookla стоят у российских провайдеров, их адреса лежат в `testip.lst`, поэтому убрать `speedtest.net` из direct недостаточно: замер всё равно шёл бы по прямой линии |
